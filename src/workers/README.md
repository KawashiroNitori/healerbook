...
- `middleware/` —— `requireAuth`（JWT 必需，401 短路）、`readAuthFromHeader`（可选读取，无副作用解析 JWT，失败时如何处理由调用方决定）、`requireSyncToken`（内部/同步端点鉴权）。
- `db/` —— `editors.ts`：`timeline_editors` / `timeline_edit_requests` 的 D1 数据访问层（DAL），参数收窄为 `D1Database`，Hono 路由与 Durable Object 两侧共用。
- `durable/` —— `TimelineDoc.ts`，协作文档的 Durable Object 实现；`stub.ts` 封装 `docStub(env, id)` 获取 DO stub。
- `collab/` —— 协作同步协议与存储：`doSqlStore.ts`（DO 内 SQLite 双表存储：snapshot + 增量 updates）等。
- 顶层其余文件：`jwt.ts`（JWT 签发/验证）、`env.ts`（Env 类型定义）、`scheduled.ts`（Cron 定时任务入口）、`allowedOrigins.ts`（CORS 白名单）、`kvKeys.ts`（KV key 构造单源）、`encounterStats.ts`（副本统计样本聚合与百分位计算）、`encounterTemplate.ts`（副本模板构建）、`top100Sync.ts`（TOP100 同步编排 + FFLogs 数据提取）、`fflogsClientV2.ts` / `samplesQueue.ts` / `sensitiveWordFilter.ts` / `userCredentials.ts` 等业务/工具模块。
