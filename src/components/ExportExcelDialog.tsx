...
export default function ExportExcelDialog({ open, onClose }: ExportExcelDialogProps) {
  ...
  useEffect(() => {
    if (open && timeline) {
      setFileName(timeline.name || t('editor:exportExcel.defaultFileName'))
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast.success(t('editor:exportExcel.exportSuccess'))
      onClose()
    } catch (err) {
      console.error('导出失败', err)
      toast.error(t('editor:exportExcel.exportFailed'))
    } finally {
  ...
  return (
    <Modal open={open} onClose={onClose} disableBackdropClick={isExporting}>
      <ModalContent>
        <ModalHeader>
          <ModalTitle>{t('editor:exportExcel.title')}</ModalTitle>
        </ModalHeader>

        <div className="space-y-5">
          {/* 文件名 */}
          <div className="space-y-1.5">
            <label className="text-sm font-medium">{t('editor:exportExcel.fileNameLabel')}</label>
            <input
              type="text"
              value={fileName}
              ...
              placeholder={t('editor:exportExcel.fileNamePlaceholder')}
            />
          </div>
          ...
          {sortedPlayers.length > 0 && (
            <div className="space-y-1.5">
              <div className="text-sm font-medium">{t('editor:exportExcel.compositionLabel')}</div>
          ...
          {/* 显示列开关 */}
          <div className="space-y-1.5">
            <div className="text-sm font-medium">{t('editor:exportExcel.columnsLabel')}</div>
            <div className="flex items-center gap-6 px-2 py-1">
              <label className="flex items-center gap-2 cursor-pointer">
                <Switch
                  ...
                  disabled={isExporting}
                />
                <span className="text-sm">{t('editor:exportExcel.originalDamage')}</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <Switch
                  ...
                  disabled={isExporting}
                />
                <span className="text-sm">{t('editor:exportExcel.actualDamage')}</span>
              </label>
            </div>
          </div>
        </div>
          ...
          <Button onClick={handleExport} disabled={isExporting || !timeline}>
            {isExporting ? (
              <>
                ...
                {t('editor:exportExcel.exporting')}
              </>
            ) : (
              <>
                <Download className="mr-1.5 h-4 w-4" />
                {t('editor:exportExcel.exportButton')}
              </>
            )}
          </Button>
        </ModalFooter>
