...
游戏事实：炽天 120s CD ≫ 慰藉 30s × 2 层 = 60s 全消耗窗口；每次炽天触发时慰藉必定满层。所以"buff 结束后残留充能是否清零"在真实战斗里触发不到，**不用引入"状态关联资源"第四种机制**。

落地：

- **废弃假 buff `20016546`**
- 慰藉 `placement` 改用 `whileStatus(3095)`（炽天真 buff，同 22s、同 executor 写入，`whileStatus` 已按 `sourcePlayerId === playerId` 过滤 → 多 SCH 天然互不干扰）
- 充能数完全由 `sch:consolation` 资源承担
...
### 慰藉无 regen 示例（炽天 @ t=120 +2，慰藉 @ t=125 −1）

查询第 2 条慰藉的 legal：

| 源                | 贡献 forbid              | 推导                       |
| ----------------- | ------------------------ | -------------------------- |
| 自耗尽            | `(−∞, 120) ∪ [125, ...)` | 实际是 amount<1 的全部时段 |
| 下游 t=125（M=0） | `(−∞, 125)`              | 无 regen → 窗口延到 −∞     |

legal 段由 amount ≥ 1 且不会透支下游决定。这里 placement 层 `whileStatus(3095)` 已经把窗口收成 `[120, 142)`，实际有效 legal ≈ 在该 buff 窗口内且 amount ≥ 1 的子集，最终即 `[125, 142)` 一段（第 1 条慰藉后 → 剩 1 层可再用一次）。
...
| 多充能有 regen，此 cast 后**仍有库存**（献奉第 1 层） | ≥1              | null（不画）                                                      |
| 多充能有 regen，此 cast 后**打空**（献奉第 2 层）     | 0               | 扫到第一个恢复到 ≥1 的 refill 时刻                                |
| 无 regen，此 cast 后仍有库存（慰藉第 1 层）           | ≥1              | null                                                              |
| 无 regen，此 cast 后打空（慰藉第 2 层）               | 0               | 扫到下一个 +delta 产出事件（下次炽天）；若无 → +∞，截到时间轴右端 |
...
例：献奉 #2 蓝条 `[37, 60)`，rawEnd − t_C = 30 → 文本 `"30s"`；慰藉 #2 蓝条 `[130, 240)`（无 regen 到下个炽天）→ 文本 `"110s"`；200+ 单充能 action rawEnd − t_C = cooldown → 文本 `"${cd}s"`，现状保留。
...
// 慰藉 (16546)
{
  ...
  placement: whileStatus(3095),                  // 改用真炽天 buff 做窗口门
