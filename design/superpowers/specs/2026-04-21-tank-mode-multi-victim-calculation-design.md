...
- 过滤规则基于 `category`（见下）
- PropertyPanel 展示 per-tank 多结果；盾值消耗等持久化 state 只认"第一个坦克"分支
...
- `maxDamage` ← `max(perVictim.finalDamage)`（保留"最坏情况"历史语义）
...
- 分支内 Phase 3 的 `meta.isTankOnly !== includeTankOnly` 被 `isStatusValidForTank` 取代（partywide 盾因此可以正确参与坦专事件）
- 只有 `perVictim[0]` 的 `updatedPartyState` 对外生效
...
- `CalculationResult.referenceMaxHP` 多坦路径取 `perVictim[0].referenceMaxHP`，单路径取 calculator 内算出的该值
...
1. **非第一坦的真实盾消耗不持久化**：OT 分支消耗了 TBN / 至黑之夜 等持久盾，MT 分支没吃到 → 对外持久化的 state 以 MT 分支为准，OT 的消耗"丢失"。按用户选择接受（简单方案）。`onBeforeShield` 每事件重算的 transient barrier（死斗 / 行尸走肉 / 出死入生）不受影响。
2. **`finalDamage` 聚合取第一坦 vs max**：选第一坦，和 `updatedPartyState` 同分支保持一致。对"最大伤害"的展示需要看 `maxDamage` 或 `perVictim`。
...
5. **partywide 盾开始参与坦专事件**（刻意的行为变化）：旧 Phase 3 用 `meta.isTankOnly !== includeTankOnly` 把 `partywide` 盾挡在 tankbuster 之外；新多坦路径按 `isStatusValidForTank` 放行 partywide 盾，第一坦分支按"一份"消耗（与 aoe 侧现行"一份"语义保持同构）。
...
5. **持久化 state 只来自 MT 分支**：OT 分支消耗的持久盾在返回的 `updatedPartyState` 中未扣减
