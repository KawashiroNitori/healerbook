...
**`excludeCastEventId` 的统一语义**：所有查询方法支持这个可选参数，实现"假设该 cast 不存在"。由于被排除的 cast 可能消耗、清除或刷新由其他 cast 附加的状态，仅靠"按 `sourceCastEventId` 过滤已冻结的 `statusTimelineByPlayer`"无法还原状态演化——例如节制附加的 `DIVINE_CARESS_READY` 会被神爱抚收束 `to`，但该 interval 的 `sourceCastEventId` 是节制的而非神爱抚的，按神爱抚 id 过滤无法恢复原时长。因此引擎按需用 `castEvents.filter(e => e.id !== excludeId)` 重跑一次 `simulate()`，产出对应的 `statusTimelineByPlayer`，再走常规查询。同一 excludeId 在 engine 生命周期内缓存，同一次拖拽中的 `dragBounds` 快照与红边框 useMemo 共享结果。
...
| engine 内部 `defaultTimeline: statusTimelineByPlayer`                 | engine 构造时计算一次      |
| engine 内部 `excludedTimelineCache: Map<castEventId, Timeline>`       | engine 构造时重置          |
...
`simulate` 需保留纯函数可调用形态：输入一组 `castEvents`（及必要的元数据），输出 `{ statusTimelineByPlayer, ... }`。PlacementEngine 在处理 `excludeCastEventId` 时会以过滤后的 `castEvents` 再调用一次，按 excludeId 缓存结果。

### 4.2 核心查询算法

```
buildContext(playerId, excludeId):
  timeline = excludeId
    ? excludedTimelineCache.getOrCompute(excludeId, () =>
        simulate(castEvents.filter(e => e.id !== excludeId)).statusTimelineByPlayer)
    : defaultTimeline
  effectiveCastEvents = excludeId
    ? castEvents.filter(e => e.id !== excludeId)
    : castEvents
  return {
    action, playerId, castEvent, actions,
...
findInvalidCastEvents(excludeId):
  ...
  for each castEvent in castEvents.filter(e => e.id !== excludeId):
    ...
    t = castEvent.timestamp
    ctx = buildContext(castEvent.playerId, excludeId, castEvent)
    // cooldown 判定必须排除当前被回溯的 castEvent 自身，否则它自己的
    // [timestamp, timestamp + cooldown) 会把自己挡掉产生假阳性。
    ctxForCooldown = { ...ctx, castEvents: ctx.castEvents.filter(e => e.id !== castEvent.id) }

    placementOk = !action.placement ||
      action.placement.validIntervals(ctx).some(i => i.from <= t && t < i.to)
    cooldownOk = cooldownAvailable(action, ctxForCooldown).some(i => i.from <= t && t < i.to)

    if placementOk && cooldownOk: continue
    reason = (!placementOk && !cooldownOk) ? 'both'
           : !placementOk ? 'placement_lost'
           : 'cooldown_conflict'
...
注意：传入 ctx 时，`statusTimelineByPlayer` 已经由 engine 按 excludeId 选择（默认快照或重放结果），combinator 无需再关心 excludeId。
...
const invalidMap = useMemo(
  () =>
    new Map(
      engine.findInvalidCastEvents(draggingId ?? undefined).map(r => [r.castEvent.id, r.reason])
    ),
  [engine, draggingId]
)
```

`CastEventIcon` 根据 `invalidMap.get(castEvent.id)` 区分 tooltip 文案，全部配红色描边：

- `placement_lost` → "此位置已不满足条件"
- `cooldown_conflict` → "与同轨其他技能 CD 冲突"
- `both` → "此位置条件不满足且与 CD 冲突"

不自动删除，用户自行调整。

拖拽父 cast 时，子 cast 的红边框实时预览（依赖 excludeId 重放）。
...
| `src/utils/mitigationCalculator.ts`             | `simulate()` 输出 `statusTimelineByPlayer`；status attach/expire/consume 记录 `sourceCastEventId` / `sourcePlayerId`；保持 `simulate` 纯函数形态，可接受任意 `castEvents` 列表并返回 timeline（供 engine 处理 excludeId 时重放）                                                                                                                                  |
...
| `src/components/Timeline/index.tsx`             | 删除 298-322 `displayActionOverrides`；新增 `engine = useMemo(createPlacementEngine, ...)` + `draggingId` state；engine 构造接收 `simulate` 函数引用（由 `MitigationCalculator` 暴露），而非预计算的 `statusTimelineByPlayer`；`addCastAt` / `handleDoubleClickTrack` / `handleCastEventDragEnd` 走 engine API；新增 `invalidCastEventMap` useMemo（保留 reason） |
...
- `findInvalidCastEvents` 识别失效 castEvent：`placement_lost` / `cooldown_conflict` / `both` 三种 reason 正确区分
- `findInvalidCastEvents` 对无 `placement` 的 action 也能识别 CD 冲突（不按 placement 短路）
- `excludeCastEventId` 重放语义：consume 型 follow-up——排除神爱抚 castEvent 后，`DIVINE_CARESS_READY` 的 interval 应恢复到节制附加的原始时长
- `excludeCastEventId` 缓存命中：同一 excludeId 的多次查询只触发一次 `simulate` 调用
...
- 拖拽神爱抚 castEvent 时，`dragBoundFunc` 的边界为节制附加的完整 `DIVINE_CARESS_READY` 窗口（验证 excludeId 重放，不受神爱抚自身消耗收束影响）
- 删除父节制后，其关联的神爱抚红边框显示
